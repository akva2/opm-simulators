...
public:
    ...
    void beginEpisode()
    {
        ...
        bool tuningEvent = this->beginEpisode_(enableExperiments, simulator.episodeIndex());
    ...
    {
        if (!this->vapparsActive(this->simulator().episodeIndex()))
            return 0.0;
...
private:
    ...
    void updateCompositionChangeLimits_()
    {
        ...
        if (this->drsdtConvective_(simulator.episodeIndex())) {
        ...
        if (this->drsdtActive_(this->simulator().episodeIndex())) {
            ElementContext elemCtx(simulator);
            const auto& vanguard = simulator.vanguard();
            auto elemIt = vanguard.gridView().template begin</*codim=*/0>();
            const auto& elemEndIt = vanguard.gridView().template end</*codim=*/0>();
        ...
        // and overlap regions
        if (this->drvdtActive_(simulator.episodeIndex())) {
            ElementContext elemCtx(simulator);
            const auto& vanguard = simulator.vanguard();
            auto elemIt = vanguard.gridView().template begin</*codim=*/0>();
            const auto& elemEndIt = vanguard.gridView().template end</*codim=*/0>();
    ...
    bool updateMaxOilSaturation_()
    {
        ...
        // we use VAPPARS
        if (this->vapparsActive(simulator.episodeIndex())) {
            ElementContext elemCtx(simulator);
            const auto& vanguard = simulator.vanguard();
            auto elemIt = vanguard.gridView().template begin</*codim=*/0>();
            const auto& elemEndIt = vanguard.gridView().template end</*codim=*/0>();
