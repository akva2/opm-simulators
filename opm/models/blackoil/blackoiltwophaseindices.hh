...
struct BlackOilTwoPhaseIndices
{
     ...
     */
    static constexpr unsigned waterSwitchIdx  = waterEnabled ? PVOffset + 0 : std::numeric_limits<unsigned>::max();
     ...
     */
    static constexpr unsigned pressureSwitchIdx  = waterEnabled ? PVOffset + 1 : PVOffset + 0;
    ...
    static constexpr unsigned compositionSwitchIdx =
        gasEnabled && oilEnabled ? PVOffset + 1 : std::numeric_limits<unsigned>::max();

    //! Index of the primary variable for the first solvent
    static constexpr unsigned solventSaturationIdx =
        enableSolvent ? PVOffset + numPhases : std::numeric_limits<unsigned>::max();

    //! Index of the primary variable for the first extbo component
    static constexpr unsigned zFractionIdx =
        enableExtbo ? PVOffset + numPhases + numSolvents : std::numeric_limits<unsigned>::max();
    ...
    static constexpr unsigned microbialConcentrationIdx =
        enableBiofilm ? PVOffset + numPhases + numSolvents : std::numeric_limits<unsigned>::max();

    //! Index of the primary variable for the biofilm component
    static constexpr unsigned biofilmVolumeFractionIdx =
        enableBiofilm ? PVOffset + numPhases + numSolvents + 1 : std::numeric_limits<unsigned>::max();
