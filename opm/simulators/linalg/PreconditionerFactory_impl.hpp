...
struct StandardPreconditioners
{
    static void add()
    {
        ...
        F::addCreator("CUILU0", [](const O& op, const P& prm, const std::function<V()>&, std::size_t, const C& comm) {
            ...
            using CuILU0 = typename cuistl::CuSeqILU0<M, cuistl::CuVector<field_type>, cuistl::CuVector<field_type>>;
        ...
        F::addCreator("CUJac", [](const O& op, const P& prm, const std::function<V()>&, std::size_t, const C& comm) {
            const double w = prm.get<double>("relaxation", 1.0);
            using field_type = typename V::field_type;
            using CuJac = typename cuistl::CuJac<M, cuistl::CuVector<field_type>, cuistl::CuVector<field_type>>;
    ...
    static void add()
    {
        ...
        if constexpr (std::is_same_v<O, Dune::MatrixAdapter<M, V, V>>) {
            ...
            F::addCreator("famg", [](const O& op, const P& prm, const std::function<V()>&, std::size_t) {
                if constexpr (std::is_same_v<typename V::field_type,double>) {
                    auto crit = AMGHelper<O,C,M,V>::criterion(prm);
        ...
        if constexpr (std::is_same_v<O, WellModelMatrixAdapter<M, V, V, false>>) {
            F::addCreator("cprw", [](const O& op, const P& prm, const std::function<V()>& weightsCalculator, std::size_t pressureIndex) {
                ...
                using Scalar = typename V::field_type;
                using LevelTransferPolicy = PressureBhpTransferPolicy<O, Dune::Amg::SequentialInformation, Scalar, false>;
        ...
        F::addCreator("CUILU0", [](const O& op, const P& prm, const std::function<V()>&, std::size_t) {
            ...
            using field_type = typename V::field_type;
            using CuILU0 = typename cuistl::CuSeqILU0<M, cuistl::CuVector<field_type>, cuistl::CuVector<field_type>>;
            return std::make_shared<cuistl::PreconditionerAdapter<V, V, CuILU0>>(std::make_shared<CuILU0>(op.getmat(), w));
        });
        ...
        F::addCreator("CUILU0Float", [](const O& op, const P& prm, const std::function<V()>&, std::size_t) {
            ...
            using CuILU0 = typename cuistl::CuSeqILU0<matrix_type_to, cuistl::CuVector<float>, cuistl::CuVector<float>>;
        ...
        F::addCreator("CUJac", [](const O& op, const P& prm, const std::function<V()>&, std::size_t) {
            ...
            using field_type = typename V::field_type;
            using CUJac = typename cuistl::CuJac<M, cuistl::CuVector<field_type>, cuistl::CuVector<field_type>>;
            return std::make_shared<cuistl::PreconditionerAdapter<V, V, CUJac>>(std::make_shared<CUJac>(op.getmat(), w));
        });
...
#define INSTANCE_PF_PAR(Scalar,Dim) \
    template class PreconditionerFactory<OpBSeq<Scalar,Dim>,CommPar>; \
    template class PreconditionerFactory<OpFPar<Scalar,Dim>,CommPar>; \
    template class PreconditionerFactory<OpBPar<Scalar,Dim>,CommPar>; \
    template class PreconditionerFactory<OpW<Scalar,Dim,false>,CommPar>; \
    template class PreconditionerFactory<OpWG<Scalar,Dim,true>,CommPar>; \
    template class PreconditionerFactory<OpBPar<Scalar,Dim>,CommSeq>;
#endif

#define INSTANCE_PF_SEQ(Scalar,Dim) \
    template class PreconditionerFactory<OpFSeq<Scalar,Dim>,CommSeq>; \
    template class PreconditionerFactory<OpBSeq<Scalar,Dim>,CommSeq>; \
    template class PreconditionerFactory<OpW<Scalar,Dim,false>,CommSeq>; \
    template class PreconditionerFactory<OpWG<Scalar,Dim,true>,CommSeq>;

#if HAVE_MPI
#define INSTANCE_PF(Scalar,Dim) \
    INSTANCE_PF_PAR(Scalar,Dim) \
    INSTANCE_PF_SEQ(Scalar,Dim)
#else
#define INSTANCE_PF(Scalar,Dim) \
    INSTANCE_PF_SEQ(Scalar,Dim)
#endif
