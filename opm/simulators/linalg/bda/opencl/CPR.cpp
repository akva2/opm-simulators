...
CPR<Scalar,block_size>::CPR(int verbosity_, bool opencl_ilu_parallel_)
...
{
    return std::abs(*std::max_element(data, data + N, [](Scalar a, Scalar b){return std::fabs(a) < std::fabs(b);}));
}
...
void CPR<Scalar,block_size>::opencl_upload()
{
    ...
    err |= queue->enqueueWriteBuffer(*d_weights, CL_FALSE, 0, sizeof(Scalar) * N, weights.data(), nullptr, &events[0]);
    for (unsigned int i = 0; i < Rmatrices.size(); ++i) {
        d_Amatrices[i].upload(queue.get(), &Amatrices[i]);

        err |= queue->enqueueWriteBuffer(d_invDiags[i], CL_FALSE, 0, sizeof(Scalar) * Amatrices[i].N, invDiags[i].data(), nullptr, &events[2*i+1]);
...
template<class Scalar, unsigned int block_size>
void CPR<Scalar,block_size>::
amg_cycle_gpu(const int level, cl::Buffer &y, cl::Buffer &x)
{
...
template<class Scalar, unsigned int block_size>
void CPR<Scalar,block_size>::
apply_amg(const cl::Buffer& y, cl::Buffer& x)
{
    // 0-initialize u and x vectors
    events.resize(d_u.size() + 1);
    err = queue->enqueueFillBuffer(*d_coarse_x, 0, 0, sizeof(Scalar) * Nb, nullptr, &events[0]);
    for (unsigned int i = 0; i < d_u.size(); ++i) {
        err |= queue->enqueueFillBuffer(d_u[i], 0, 0, sizeof(Scalar) * Rmatrices[i].N, nullptr, &events[i + 1]);
    }
    cl::WaitForEvents(events);
    events.clear();
    if (err != CL_SUCCESS) {
    ...
    OpenclKernels<Scalar>::residual(d_mat->nnzValues, d_mat->colIndices, d_mat->rowPointers, x, y, *d_rs, Nb, block_size);
