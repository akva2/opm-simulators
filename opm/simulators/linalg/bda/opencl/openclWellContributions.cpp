...
void WellContributionsOCL<Scalar>::setOpenCLEnv(cl::Context* context_, cl::CommandQueue* queue_)
{
...
{
    OpenclKernels<Scalar>::apply_stdwells(*d_Cnnzs_ocl, *d_Dnnzs_ocl, *d_Bnnzs_ocl, *d_Ccols_ocl, *d_Bcols_ocl,
                                          d_x, d_y, this->dim, this->dim_wells, *d_val_pointers_ocl, this->num_std_wells);
}
...
void WellContributionsOCL<Scalar>::apply_mswells(cl::Buffer d_x, cl::Buffer d_y)
{
    ...
    events.resize(2);
    queue->enqueueReadBuffer(d_x, CL_FALSE, 0, sizeof(Scalar) * this->N, h_x.data(), nullptr, &events[0]);
    queue->enqueueReadBuffer(d_y, CL_FALSE, 0, sizeof(Scalar) * this->N, h_y.data(), nullptr, &events[1]);
    ...
    events.resize(1);
    queue->enqueueWriteBuffer(d_y, CL_FALSE, 0, sizeof(Scalar) * this->N, h_y.data(), nullptr, &events[0]);
    events[0].wait();
    events.clear();
}
...
template<class Scalar>
void WellContributionsOCL<Scalar>::APIaddMatrix(MatrixType type,
                                                int* colIndices,
                                                Scalar* values,
                                                unsigned int val_size)
{
    ...
    case MatrixType::C:
        events.resize(2);
        queue->enqueueWriteBuffer(*d_Cnnzs_ocl, CL_FALSE, sizeof(Scalar) * this->num_blocks_so_far * this->dim * this->dim_wells,
                                  sizeof(Scalar) * val_size * this->dim * this->dim_wells,
                                  values, nullptr, &events[0]);
        queue->enqueueWriteBuffer(*d_Ccols_ocl, CL_FALSE, sizeof(int) * this->num_blocks_so_far,
    ...
    case MatrixType::D:
        events.resize(1);
        queue->enqueueWriteBuffer(*d_Dnnzs_ocl, CL_FALSE, sizeof(Scalar) * this->num_std_wells_so_far * this->dim_wells * this->dim_wells,
                                  sizeof(Scalar) * this->dim_wells * this->dim_wells, values, nullptr, &events[0]);
        events[0].wait();
        events.clear();
        break;
    ...
    case MatrixType::B:
        events.resize(2);
        queue->enqueueWriteBuffer(*d_Bnnzs_ocl, CL_FALSE, sizeof(Scalar) * this->num_blocks_so_far * this->dim * this->dim_wells,
                                  sizeof(Scalar) * val_size * this->dim * this->dim_wells, values, nullptr, &events[0]);
        queue->enqueueWriteBuffer(*d_Bcols_ocl, CL_FALSE, sizeof(int) * this->num_blocks_so_far,
                                  sizeof(int) * val_size, colIndices, nullptr, &events[1]);
        cl::WaitForEvents(events);
        events.clear();
        ...
        if (this->num_std_wells_so_far == this->num_std_wells - 1) {
            ...
            events.resize(1);
            queue->enqueueWriteBuffer(*d_val_pointers_ocl, CL_FALSE, 0, sizeof(unsigned int) * (this->num_std_wells + 1),
...
void WellContributionsOCL<Scalar>::APIalloc()
{
    ...
    d_val_pointers_ocl = std::make_unique<cl::Buffer>(*context, CL_MEM_READ_WRITE, sizeof(unsigned int) * (this->num_std_wells + 1));
}
