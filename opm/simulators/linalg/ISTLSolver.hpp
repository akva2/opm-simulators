...
std::unique_ptr<Matrix> blockJacobiAdjacency(const Grid& grid,
    ...
    public:
        ...
        void initialize()
        {
            ...
            if (parameters_[0].linsolver_ == "hybrid") {
                ...
                parameters_.clear();
                {
                    ...
                    prm_.push_back(setupPropertyTree(parameters_[0],
                                                     Parameters::isSet<Properties::LinearSolverMaxIter>(),
                                                     Parameters::isSet<Properties::LinearSolverReduction>()));
                }
                {
                    FlowLinearSolverParameters para;
                    para.init<TypeTag>(false);
            ...
            useWellConn_ = Parameters::get<Properties::MatrixAddWellContributions>();
            const bool ownersFirst = Parameters::get<Properties::OwnerCellsFirst>();
            if (!ownersFirst) {
