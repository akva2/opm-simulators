...
public:
    ...
    AquiferCarterTracy(const std::vector<Aquancon::AquancCell>& connections,
        ...
        : Base(aquct_data.aquiferID, connections, modelSimulator)
        , aquct_data_(aquct_data)
    {}

    static AquiferCarterTracy serializationTestObject(const Simulator& modelSimulator)
    {
        AquiferCarterTracy result({}, modelSimulator, {});
    ...
    void endTimeStep() override
    {
        for (const auto& q : this->Qai_) {
            this->W_flux_ += q * this->model_simulator_.timeStepSize();
        }
...
protected:
    ...
    void calculateAquiferCondition() override
    {
        ...
        if (! this->aquct_data_.initial_pressure.has_value()) {
            ...
            const auto& tables = this->model_simulator_.vanguard()
