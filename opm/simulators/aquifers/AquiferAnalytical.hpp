...
public:
    ...
    AquiferAnalytical(const int aqID,
        ...
        : AquiferInterface<TypeTag>(aqID, modelSimulator)
    ...
    {
        ElementContext elemCtx(this->model_simulator_);
        OPM_BEGIN_PARALLEL_TRY_CATCH();

        for (const auto& elem : elements(this->model_simulator_.gridView())) {
    ...
    {
        const auto& model = this->model_simulator_.model();
        ...
        this->calculateInflowRate(idx, this->model_simulator_);
        ...
        if constexpr (enableEnergy) {
            ...
            if (this->Ta0_.has_value() && this->Qai_[idx] > 0)
            {
                ...
                paramCache.setMaxOilSat(this->model_simulator_.problem().maxOilSaturation(cellIdx));
...
protected:
    ...
    {
        return this->model_simulator_.problem().gravity()[2];
    }
    ...
    void initializeConnectionMappings()
    {
        ...
        this->cellToConnectionIdx_.resize(this->model_simulator_.gridView().size(/*codim=*/0), -1);
        const auto& gridView = this->model_simulator_.vanguard().gridView();
        for (std::size_t idx = 0; idx < this->size(); ++idx) {
            const auto global_index = this->connections_[idx].global_index;
            const int cell_index = this->model_simulator_.vanguard().compressedIndex(global_index);
            if (cell_index < 0) {
                continue;
            }
        ...
        const auto& elemMapper = this->model_simulator_.model().dofMapper();
    ...
    void initializeConnectionDepths()
    {
        ...
        const auto& gridView = this->model_simulator_.vanguard().gridView();
        for (std::size_t idx = 0; idx < this->size(); ++idx) {
            const int cell_index = this->model_simulator_.vanguard()
            ...
            this->cell_depth_.at(idx) = this->model_simulator_.vanguard().cellCenterDepth(cell_index);
        }
    }
