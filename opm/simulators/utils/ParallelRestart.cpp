...
std::pair<std::set<Type>, std::vector<std::pair<Key, std::vector<int>>>>
splitDynMap(const Map<Key, Opm::DynamicState<Type>>& map)
{
    ...
    for (const auto& it : map) {
        ...
        for (const auto& w : it.second.data()) {
            int distance = std::distance(unique.begin(), unique.find(w));
            idxVec.push_back(distance);
        }
...
template<class Type>
std::pair<std::set<Type>, std::vector<int>>
...
template<class Type>
void reconstructDynState(const std::set<Type>& unique,
                         const std::vector<int>& idxVec,
                         Opm::DynamicState<Type>& result)
{
    std::vector<Type> ptrData;
    for (size_t i = 0; i < idxVec.size()-1; ++i) {
        auto wIt = unique.begin();
        std::advance(wIt, idxVec[i]);
        ptrData.push_back(*wIt);
    }
    result = Opm::DynamicState<Type>(ptrData, idxVec.back());
}

template<template<class, class> class Map, class Type, class Key>
void reconstructDynMap(const std::set<Type>& unique,
