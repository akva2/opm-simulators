...
public:
    ...
    virtual void equilInitialCondition_(EclMaterialLawManager& materialLawManager,
                                        const Simulator& simulator,
                                        const std::size_t numElems) = 0;

    //! \brief Read explicitly specified initial conditions from eclipse state.
    virtual void explicitInitialCondition_(const FieldPropsManager& fp,
                                           const EclMaterialLawManager& materialLawManager,
                                           const std::size_t numDof,
                                           std::function<int(int)> pvtRegionIndex) = 0;
