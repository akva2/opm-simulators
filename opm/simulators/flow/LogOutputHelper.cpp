                              ...
                              const std::vector<std::string>& wellCumNames) const
{
    ...
    ss << fmt::format("{:>8}:{:>4}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:",
                          ...
                          const int reg) const
{
    ...
    if (reg == 0) {
        ...
        ss << fmt::format("{: >{}}:{:^48}:\n", "", 53,
                          name + " REPORT REGION " + std::to_string(reg));
    }
    ss << fmt::format("{: >{}}:{:>14}{:>14.0f}  {:<18}:\n",
                      "", 53,
                      "PAV  =",
                      pav,
                      units.name(UnitSystem::measure::pressure))
       << fmt::format("{: >{}}:{:>14}{:>14.0f}  {:<18}:\n",
                      "", 53, "PORV =",
                      cip[Inplace::Phase::PoreVolume],
    ...
    {
        return fmt::format(" :{:<25}:{:^15.0f}{:^14.0f}{:^14.0f}:{:^16.0f}:{:^15.0f}{:^14.0f}{:^14.0f}:\n",
                           label,
    ...
    ss << fmt::format("{0: >27}:{0:->15} OIL {1:>4} {0:->18}:"
                      "-- WAT{0: >2} {1:>4} --:{0:->15} GAS{0: >3} {2:>4} {0:-^15}:\n",
                      "",
                      units.name(UnitSystem::measure::liquid_surface_volume),
                      units.name(UnitSystem::measure::gas_surface_volume))
       << fmt::format("{0: >27}:{1:^15}{2:^14}{3:^14}:{3:^16}:{4:^15}{5:^14}{3:^14}:\n",
                      "",
                      "LIQUID",
                      "VAPOUR",
                      "TOTAL",
