...
public:
    ...
    BlackoilModelNldd(BlackoilModel<TypeTag>& model)
        : model_(model), rank_(model_.modelSimulator().vanguard().grid().comm().rank())
    {
        // Create partitions.
        ...
        const auto& grid = model_.modelSimulator().vanguard().grid();
        ...
        for (int index = 0; index < num_domains; ++index) {
            ...
            const auto& eclState = model_.modelSimulator().vanguard().eclState();
            ...
            domain_linsolvers_.emplace_back(model_.modelSimulator(), loc_param, force_serial);
        }
                                                 ...
                                                 NonlinearSolverType& nonlinear_solver)
    {
        ...
        auto& solution = model_.modelSimulator().model().solution(0);
        ...
        auto global_logger = gatherDeferredLogger(logger, model_.modelSimulator().vanguard().grid().comm());
        ...
        if (model_.param().local_solve_approach_ == DomainSolveApproach::Jacobi) {
            ...
            model_.modelSimulator().model().invalidateAndUpdateIntensiveQuantities(/*timeIdx=*/0);
        }
        ...
        // we move on.
        const auto& comm = model_.modelSimulator().vanguard().grid().comm();
        if (comm.size() > 1) {
            const auto* ccomm = model_.modelSimulator().model().newtonMethod().linearSolver().comm();
            ...
            model_.modelSimulator().model().invalidateAndUpdateIntensiveQuantitiesOverlap(/*timeIdx=*/0);
    ...
    {
        const auto& elementMapper = this->model_.modelSimulator().model().elementMapper();
        const auto& cartMapper = this->model_.modelSimulator().vanguard().cartesianIndexMapper();

        const auto& grid = this->model_.modelSimulator().vanguard().grid();
...
private:
    ...
    SimulatorReportSingle assembleReservoirDomain(const Domain& domain)
    {
        ...
        model_.modelSimulator().model().linearizer().linearizeDomain(domain);
    ...
    {
        const auto& modelSimulator = model_.modelSimulator();
                           ...
                           const Domain& domain)
    {
        ...
        if (local_report.converged) {
            ...
            model_.modelSimulator().model().invalidateAndUpdateIntensiveQuantities(/*timeIdx=*/0, domain);
        } else {
    ...
    decltype(auto) partitionCells() const
    {
        ...
        zoltan_ctrl.index =
            [elementMapper = &this->model_.modelSimulator().model().elementMapper()]
            (const Element& element)
        {
            return elementMapper->index(element);
        };
        zoltan_ctrl.local_to_global =
            [cartMapper = &this->model_.modelSimulator().vanguard().cartesianIndexMapper()]
            (const int elemIdx)
        {
        ...
        const auto wells = need_wells
            ? this->model_.modelSimulator().vanguard().schedule().getWellsatEnd()
            : std::vector<Well>{};
