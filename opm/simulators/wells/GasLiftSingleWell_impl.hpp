...
GasLiftSingleWell(const WellInterface<TypeTag> &well,
    ...
    : GasLiftSingleWellGeneric(
        ...
        group_info,
        well.phaseUsage(),
        model_simulator.vanguard().schedule(),
        model_simulator.episodeIndex(),
        sync_groups,
        comm,
        glift_debug
    )
   , model_simulator_{model_simulator}
   , well_{well}
{
...
computeWellRates_( double bhp, bool bhp_is_limited, bool debug_output ) const
{
    ...
    this->well_.computeWellRatesWithBhp(
        this->model_simulator_, bhp, potentials, this->deferred_logger_);
    if (debug_output) {
