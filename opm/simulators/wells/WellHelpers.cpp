...
template<class Scalar, int Dim> using Vec = Dune::BlockVector<Dune::FieldVector<Scalar,Dim>>;
template<class Scalar> using DynVec = Dune::BlockVector<Dune::DynamicVector<Scalar>>;
template<class Scalar> using DMatrix = Dune::DynamicMatrix<Scalar>;

#define INSTANCE(T, Dim)                                              \
    template void ParallelStandardWellB<T>::                          \
        mv<Vec<T,Dim>,DynVec<T>>(const Vec<T,Dim>&,DynVec<T>&) const; \
    template void ParallelStandardWellB<T>::                          \
        mmv<Vec<T,Dim>,DynVec<T>>(const Vec<T,Dim>&,DynVec<T>&) const;

#define INSTANCE_TYPE(T)                                                        \
    template class ParallelStandardWellB<T>;                                    \
    template void sumDistributedWellEntries<T,Comm>(Dune::DynamicMatrix<T>&,    \
                                                     Dune::DynamicVector<T>&,   \
                                                     const Comm& comm);         \
    template DMatrix<T> transposeDenseDynMatrix<DMatrix<T>>(const DMatrix<T>&); \
    template T computeHydrostaticCorrection<T>(const T,                         \
                                               const T,                         \
                                               const T,                         \
                                               const T);                        \
    INSTANCE(T,1)                                                               \
