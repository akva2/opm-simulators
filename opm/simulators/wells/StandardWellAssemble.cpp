...
assembleControlEq(const WellState& well_state,
                  ...
                  const double rho,
                  StandardWellEquations<Indices,Scalar>& eqns,
                  DeferredLogger& deferred_logger) const
{
    ...
    eqns.resWell_[0][eqns.Bhp] = control_eq.value();
    for (int pv_idx = 0; pv_idx < numWellEq; ++pv_idx) {
        eqns.duneD_[0][0][eqns.Bhp][pv_idx] = control_eq.derivative(pv_idx + Indices::numEq);
    }
}
