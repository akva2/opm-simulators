...
namespace Opm
{
    ...
    bool
    WellInterface<TypeTag>::
    updateWellControl(const Simulator& model_simulator,
                      ...
                      DeferredLogger& deferred_logger) /* const */
    {
        ...
        const auto& schedule = model_simulator.vanguard().schedule();
        ...
        Parallel::Communication cc = model_simulator.vanguard().grid().comm();
        ...
        if (changed) {
            ...
            this->well_control_log_.push_back(from);
            updateWellStateWithTarget(model_simulator, group_state, well_state, deferred_logger);
    ...
    bool
    WellInterface<TypeTag>::
    updateWellControlAndStatusLocalIteration(const Simulator& model_simulator,
                                             WellState& well_state,
                                             const GroupState& group_state,
    ...
    bool
    WellInterface<TypeTag>::
    iterateWellEquations(const Simulator& modelSimulator,
                         const double dt,
                         WellState& well_state,
                         const GroupState& group_state,
                         DeferredLogger& deferred_logger)
    {
        ...
        try {
            ...
            if (!this->param_.local_well_solver_control_switching_){
                converged = this->iterateWellEqWithControl(modelSimulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger);
            } else {
                if (this->param_.use_implicit_ipr_ && this->well_ecl_.isProducer() && this->wellHasTHPConstraints(summary_state) && (this->well_ecl_.getStatus() == WellStatus::OPEN)) {
                    converged = solveWellWithTHPConstraint(modelSimulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger);
                } else {
                    converged = this->iterateWellEqWithSwitching(modelSimulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger);
                }
            }
    ...
    bool
    WellInterface<TypeTag>::
    solveWellWithTHPConstraint(const Simulator& model_simulator,
                               const double dt,
                               const Well::InjectionControls& inj_controls,
                               const Well::ProductionControls& prod_controls,                           
                               WellState& well_state,
                               ...
                               DeferredLogger& deferred_logger)
    {
        ...
        if (this->wellIsStopped()) {
            this->openWell();
            auto bhp_target = estimateOperableBhp(model_simulator, dt, well_state, summary_state, deferred_logger);
            if (!bhp_target.has_value()) {
                ...
                solveWellWithZeroRate(model_simulator, dt, well_state, deferred_logger);
                this->stopWell();
            } else {
                // solve well with the estimated target bhp (or limit)
                ws.thp = this->getTHPConstraint(summary_state);
                const double bhp = std::max(bhp_target.value(), prod_controls.bhp_limit);
                solveWellWithBhp(model_simulator, dt, bhp, well_state, deferred_logger);
            }
        }
        // solve well-equation
        if (is_operable) {
            converged = this->iterateWellEqWithSwitching(model_simulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger);
        }
        ...
        if (converged && !this->stopppedOrZeroRateTarget(summary_state, well_state) && isThp) {
            ...
            this->updateIPRImplicit(model_simulator, well_state, deferred_logger);
            ...
            if (!is_stable) {
                ...
                if (bhp_stable.has_value() && cur_bhp - bhp_stable.value() > cur_bhp*reltol){
                    ...
                    deferred_logger.debug(msg);
                    solveWellWithBhp(model_simulator, dt, bhp_stable.value(), well_state, deferred_logger);
        ...
        if (!converged) {
            ...
            if (!bhp_target.has_value()) {
                ...
                converged = solveWellWithZeroRate(model_simulator, dt, well_state, deferred_logger);
                this->stopWell();
            } else {      
                // solve well with the estimated target bhp (or limit)
                const double bhp = std::max(bhp_target.value(), prod_controls.bhp_limit);
    ...
    std::optional<double>
    WellInterface<TypeTag>::
    estimateOperableBhp(const Simulator& model_simulator,
                        const double dt,
                        WellState& well_state,
    ...
    {   
        ...
        // Solve
        const bool converged = solveWellWithBhp(model_simulator, dt, bhp_min, well_state, deferred_logger);
    ...
    bool
    WellInterface<TypeTag>::
    solveWellWithBhp(const Simulator& model_simulator,
                     const double dt,
                     const double bhp,
                     WellState& well_state,
                     DeferredLogger& deferred_logger)
    {   
        ...
        ws.bhp = bhp;
        // solve 
        const bool converged =  this->iterateWellEqWithSwitching(model_simulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger, /*fixed_control*/true);
    ...
    bool
    WellInterface<TypeTag>::
    solveWellWithZeroRate(const Simulator& model_simulator,
                          const double dt,
                          WellState& well_state,
                          DeferredLogger& deferred_logger)
    {   
        ...
        auto inj_controls = Well::InjectionControls(0);
        auto prod_controls = Well::ProductionControls(0); 
        const bool converged =  this->iterateWellEqWithSwitching(model_simulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger, /*fixed_control*/true, /*fixed_status*/ true);
    ...
    bool
    WellInterface<TypeTag>::
    solveWellForTesting(const Simulator& modelSimulator, WellState& well_state, const GroupState& group_state,
                        DeferredLogger& deferred_logger)
    {
        ...
        if (has_thp_limit) {
            ...
            converged = gliftBeginTimeStepWellTestIterateWellEquations(
                modelSimulator, dt, well_state, group_state, deferred_logger);
        }
        else {
            well_state.well(this->indexOfWell()).production_cmode = Well::ProducerCMode::BHP;
            converged = iterateWellEquations(modelSimulator, dt, well_state, group_state, deferred_logger);
        }
        if (converged) {
    ...
    void
    WellInterface<TypeTag>::
    solveWellEquation(const Simulator& modelSimulator,
                      WellState& well_state,
                      const GroupState& group_state,
                      DeferredLogger& deferred_logger)
    {
        ...
        const WellState well_state0 = well_state;
        const double dt = modelSimulator.timeStepSize();
        bool converged = iterateWellEquations(modelSimulator, dt, well_state, group_state, deferred_logger);
        ...
        if (!converged) {
            ...
            if (thp_control) {
                ...
                deferred_logger.debug(msg);
                converged = this->iterateWellEquations(modelSimulator, dt, well_state, group_state, deferred_logger);
            }
        }
    ...
    void
    WellInterface<TypeTag>::
    assembleWellEq(const Simulator& modelSimulator,
                   const double dt,
                   WellState& well_state,
                   const GroupState& group_state,
                   DeferredLogger& deferred_logger)
    {
        ...
        prepareWellBeforeAssembling(modelSimulator, dt, well_state, group_state, deferred_logger);

        assembleWellEqWithoutIteration(modelSimulator, dt, well_state, group_state, deferred_logger);
    }
    ...
    void
    WellInterface<TypeTag>::
    prepareWellBeforeAssembling(const Simulator& modelSimulator,
                                const double dt,
                                WellState& well_state,
                                const GroupState& group_state,
                                DeferredLogger& deferred_logger)
    {
        ...
        if (param_.check_well_operability_iter_)
            checkWellOperability(modelSimulator, well_state, deferred_logger);

        // only use inner well iterations for the first newton iterations.
        const int iteration_idx = modelSimulator.model().newtonMethod().numIterations();
        if (iteration_idx < param_.max_niter_inner_well_iter_ || this->well_ecl_.isMultiSegment()) {
            this->operability_status_.solvable = true;
            bool converged = this->iterateWellEquations(modelSimulator, dt, well_state, group_state, deferred_logger);
        ...
        if (this->operability_status_.has_negative_potentials) {
            ...
            std::vector<double> potentials;
            try {
                computeWellPotentials(modelSimulator, well_state_copy, potentials, deferred_logger);
            } catch (const std::exception& e) {
    ...
    void
    WellInterface<TypeTag>::
    checkWellOperability(const Simulator& model_simulator,
                         const WellState& well_state,
                         DeferredLogger& deferred_logger)
    {
        ...
        updateWellOperability(model_simulator, well_state, deferred_logger);
        ...
        DeferredLogger& deferred_logger)
    {
        ...
        assert(this->wellHasTHPConstraints(model_simulator.vanguard().summaryState()));
        const auto& schedule = model_simulator.vanguard().schedule();
        auto report_step_idx = model_simulator.episodeIndex();
        ...
        if(glo.active() && glo.has_well(well_name)) {
            ...
            while (alq > 0) {
                ...
                if ((converged =
                      iterateWellEquations(model_simulator, dt, well_state, group_state, deferred_logger)))
                {
                    return converged;
                }
                alq -= increment;
            ...
            return iterateWellEquations(model_simulator, dt, well_state, group_state, deferred_logger);
        }
    }
    ...
    void
    WellInterface<TypeTag>::
    gliftBeginTimeStepWellTestUpdateALQ(const Simulator& model_simulator,
                          WellState& well_state,
                          DeferredLogger& deferred_logger)
    {
        const auto& summary_state = model_simulator.vanguard().summaryState();
        ...
        const auto& schedule = model_simulator.vanguard().schedule();
        const auto report_step_idx = model_simulator.episodeIndex();
        const auto& glo = schedule.glo(report_step_idx);
    ...
    void
    WellInterface<TypeTag>::
    updateWellOperability(const Simulator& model_simulator,
                          const WellState& well_state,
                          DeferredLogger& deferred_logger)
    {
        if (this->param_.local_well_solver_control_switching_) {
            const bool success = updateWellOperabilityFromWellEq(model_simulator, well_state, deferred_logger);
            if (success) {
                return;
            } else {
        ...
        if (check_thp || bhp_controlled) {
            ...
            checkOperabilityUnderBHPLimit(well_state, model_simulator, deferred_logger);
        }
        // we do some extra checking for wells under THP control.
        if (check_thp) {
            checkOperabilityUnderTHPLimit(model_simulator, well_state, deferred_logger);
        }
    }
    ...
    bool
    WellInterface<TypeTag>::
    updateWellOperabilityFromWellEq(const Simulator& model_simulator,
                                    const WellState& well_state,
                                    DeferredLogger& deferred_logger)
    {
        ...
        bool converged = iterateWellEquations(model_simulator, dt, well_state_copy, group_state, deferred_logger);
        return converged;
    }
        ...
        else
        {
            ...
            case Well::ProducerCMode::ORAT:
            {
                ...
                if (current_rate > 0.0) {
                    ...
                    const std::vector<double> fractions = initialWellRateFractions(model_simulator, well_state);
            ...
            {
                const bool update_success = updateWellStateWithTHPTargetProd(model_simulator, well_state, deferred_logger);
    ...
    std::vector<double>
    WellInterface<TypeTag>::
    initialWellRateFractions(const Simulator& modelSimulator, const WellState& well_state) const
    {
    ...
    void
    WellInterface<TypeTag>::
    updateWellStateRates(const Simulator& modelSimulator,
                         WellState& well_state,
                         DeferredLogger& deferred_logger) const
    {
        ...
        std::vector<double> well_q_s = computeCurrentWellRates(modelSimulator, deferred_logger);
                ...
                [[maybe_unused]] DeferredLogger& deferred_logger) const
    {
        ...
        const auto& intQuants = modelSimulator.model().intensiveQuantities(cell_idx, /*timeIdx=*/0);
        const auto& materialLawManager = modelSimulator.problem().materialLawManager();
    ...
    bool
    WellInterface<TypeTag>::
    updateWellStateWithTHPTargetProd(const Simulator& model_simulator,
                                     WellState& well_state,
                                     DeferredLogger& deferred_logger) const
    {
        const auto& summary_state = model_simulator.vanguard().summaryState();
        ...
        auto bhp_at_thp_limit = computeBhpAtThpLimitProdWithAlq(
            model_simulator, summary_state, this->getALQ(well_state), deferred_logger);
        if (bhp_at_thp_limit) {
            std::vector<double> rates(this->number_of_phases_, 0.0);
            if (thp_update_iterations) {
                computeWellRatesWithBhpIterations(model_simulator, *bhp_at_thp_limit,
                                                  rates, deferred_logger);
            } else {
                computeWellRatesWithBhp(model_simulator, *bhp_at_thp_limit,
                                        rates, deferred_logger);
            }
