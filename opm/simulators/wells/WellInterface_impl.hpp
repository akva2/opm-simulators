...
namespace Opm
{
    ...
    bool
    WellInterface<TypeTag>::
    gliftBeginTimeStepWellTestIterateWellEquations(const Simulator& ebos_simulator,
                                                   const double dt,
                                                   WellState<Scalar>& well_state,
                                                   const GroupState<Scalar>& group_state,
                                                   DeferredLogger& deferred_logger)
        ...
        else
        {
            ...
            case Well::ProducerCMode::ORAT:
            {
                ...
                if (current_rate > 0.0) {
                    ...
                    const std::vector<Scalar> fractions = initialWellRateFractions(ebos_simulator, well_state);
                    Scalar control_fraction = fractions[pu.phase_pos[Oil]];
            ...
            case Well::ProducerCMode::WRAT:
            {
                ...
                if (current_rate > 0.0) {
                    ...
                    const std::vector<Scalar> fractions = initialWellRateFractions(ebos_simulator, well_state);
                    Scalar control_fraction = fractions[pu.phase_pos[Water]];
                    if (control_fraction != 0.0) {
                        for (int p = 0; p<np; ++p) {
            ...
            case Well::ProducerCMode::GRAT:
            {
                ...
                if (current_rate > 0.0) {
                    ...
                    const std::vector<Scalar> fractions = initialWellRateFractions(ebos_simulator, well_state);
                    Scalar control_fraction = fractions[pu.phase_pos[Gas]];
                    if (control_fraction != 0.0) {
                        for (int p = 0; p<np; ++p) {
            ...
            case Well::ProducerCMode::LRAT:
            {
                ...
                if (current_rate > 0.0) {
                    ...
                    const std::vector<Scalar> fractions = initialWellRateFractions(ebos_simulator, well_state);
                    Scalar control_fraction = fractions[pu.phase_pos[Water]] + fractions[pu.phase_pos[Oil]];
                    if (control_fraction != 0.0) {
                        for (int p = 0; p<np; ++p) {
                         ...
                         DeferredLogger& deferred_logger) const
    {
        ...
        std::vector<Scalar> well_q_s = computeCurrentWellRates(ebosSimulator, deferred_logger);
