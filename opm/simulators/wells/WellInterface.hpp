...
public:
    ...
    void assembleWellEq(const Simulator& modelSimulator,
                        const double dt,
                        WellState& well_state,
                        const GroupState& group_state,
                        DeferredLogger& deferred_logger);

    void assembleWellEqWithoutIteration(const Simulator& modelSimulator,
                                        const double dt,
                                        WellState& well_state,
                                        const GroupState& group_state,
                                        DeferredLogger& deferred_logger);
    ...
    void prepareWellBeforeAssembling(const Simulator& modelSimulator,
                                     const double dt,
                                     WellState& well_state,
                                     const GroupState& group_state,
                                     DeferredLogger& deferred_logger);
    ...
    virtual void computeWellPotentials(const Simulator& modelSimulator,
                                       const WellState& well_state,
                                       std::vector<double>& well_potentials,
                                       DeferredLogger& deferred_logger) = 0;

    virtual void updateWellStateWithTarget(const Simulator& model_simulator,
                                           const GroupState& group_state,
                                           WellState& well_state,
                                           DeferredLogger& deferred_logger) const;

    virtual void computeWellRatesWithBhpIterations(const Simulator& modelSimulator,
                                                   const Scalar& bhp,
                                                   std::vector<double>& well_flux,
                                                   DeferredLogger& deferred_logger) const = 0;

    bool updateWellStateWithTHPTargetProd(const Simulator& model_simulator,
                                          WellState& well_state,
                                          DeferredLogger& deferred_logger) const;

    enum class IndividualOrGroup { Individual, Group, Both };
    bool updateWellControl(const Simulator& model_simulator,
                           const IndividualOrGroup iog,
                           WellState& well_state,
                           const GroupState& group_state,
                           DeferredLogger& deferred_logger) /* const */;

    bool updateWellControlAndStatusLocalIteration(const Simulator& model_simulator,
                                                  WellState& well_state,
                                                  const GroupState& group_state,
    ...
    virtual void calculateExplicitQuantities(const Simulator& modelSimulator,
                                             const WellState& well_state,
                                             DeferredLogger& deferred_logger) = 0; // should be const?

    virtual void updateProductivityIndex(const Simulator& modelSimulator,
    ...
    void checkWellOperability(const Simulator& model_simulator,
                              const WellState& well_state,
                              DeferredLogger& deferred_logger);
    ...
    void gliftBeginTimeStepWellTestUpdateALQ(const Simulator& model_simulator,
                                             WellState& well_state,
                                             DeferredLogger& deferred_logger);

    // check whether the well is operable under the current reservoir condition
    // mostly related to BHP limit and THP limit
    void updateWellOperability(const Simulator& model_simulator,
                               const WellState& well_state,
                               DeferredLogger& deferred_logger);

    bool updateWellOperabilityFromWellEq(const Simulator& model_simulator,
                                         const WellState& well_state,
                                         DeferredLogger& deferred_logger);
    ...
    virtual std::vector<double> computeCurrentWellRates(const Simulator& modelSimulator,
                                                        DeferredLogger& deferred_logger) const = 0;
    ...
    void updateWellStateRates(const Simulator& modelSimulator,
                              WellState& well_state,
                              DeferredLogger& deferred_logger) const;

    void solveWellEquation(const Simulator& modelSimulator,
                           WellState& well_state,
                           const GroupState& group_state,
                           DeferredLogger& deferred_logger);
...
protected:
    ...
    std::vector<double> initialWellRateFractions(const Simulator& modelSimulator,
    ...
    virtual void checkOperabilityUnderTHPLimit(const Simulator& model_simulator,
                                               const WellState& well_state,
                                               DeferredLogger& deferred_logger) = 0;

    virtual void updateIPR(const Simulator& model_simulator,
                           DeferredLogger& deferred_logger) const=0;

    virtual void assembleWellEqWithoutIteration(const Simulator& modelSimulator,
                                                const double dt,
    ...
    virtual bool iterateWellEqWithControl(const Simulator& modelSimulator,
                                          const double dt,
                                          const WellInjectionControls& inj_controls,
                                          const WellProductionControls& prod_controls,
                                          WellState& well_state,
    ...
    virtual bool iterateWellEqWithSwitching(const Simulator& modelSimulator,
                                            const double dt,
                                            const WellInjectionControls& inj_controls,
                                            const WellProductionControls& prod_controls,
                                            WellState& well_state,
    ...
    virtual void updateIPRImplicit(const Simulator& modelSimulator,
                                   WellState& well_state,
                                   DeferredLogger& deferred_logger) = 0;                                            

    bool iterateWellEquations(const Simulator& modelSimulator,
                              const double dt,
                              WellState& well_state,
                              const GroupState& group_state,
                              DeferredLogger& deferred_logger);

    bool solveWellWithTHPConstraint(const Simulator& model_simulator,
                                    const double dt,
                                    const Well::InjectionControls& inj_controls,
                                    const Well::ProductionControls& prod_controls,                                
                                    WellState& well_state,
    ...
    std::optional<double> estimateOperableBhp(const Simulator& model_simulator,
                                              const double dt,
                                              WellState& well_state,
                                              const SummaryState& summary_state,
                                              DeferredLogger& deferred_logger);        

    bool solveWellWithBhp(const Simulator& model_simulator,
                          const double dt,
                          const double bhp,
                          WellState& well_state,
                          DeferredLogger& deferred_logger);         

    bool solveWellWithZeroRate(const Simulator& model_simulator,
                               const double dt,
                               WellState& well_state,
                               DeferredLogger& deferred_logger);                                                                                                       

    bool solveWellForTesting(const Simulator& modelSimulator,
                             WellState& well_state,
                             const GroupState& group_state,
                             DeferredLogger& deferred_logger);
    ...
    void getMobility(const Simulator& modelSimulator,
                     const int perf,
