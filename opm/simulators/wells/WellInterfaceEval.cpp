...
#define INSTANCE_METHODS(...) \
template void WellInterfaceEval<BlackOilFluidSystem<double, BlackOilDefaultIndexTraits>>:: \
getGroupInjectionControl<__VA_ARGS__>(const Group& group, \
                                      const WellState& well_state, \
                                      const GroupState& group_state, \
                                      const Schedule& schedule, \
                                      const SummaryState& summaryState, \
                                      const __VA_ARGS__& bhp, \
                                      const std::vector<__VA_ARGS__>& rates, \
                                      __VA_ARGS__& control_eq, \
                                     double efficiencyFactor); \
template void WellInterfaceEval<BlackOilFluidSystem<double, BlackOilDefaultIndexTraits>>:: \
getGroupProductionControl<__VA_ARGS__>(const Group& group, \
                          const WellState& well_state, \
                          const GroupState& group_state, \
                          const Schedule& schedule, \
                          const SummaryState& summaryState, \
...
template class WellInterfaceEval<BlackOilFluidSystem<double, BlackOilDefaultIndexTraits>>;

//INSTANCE_METHODS(DenseAd::Evaluation<double,5,0>)
