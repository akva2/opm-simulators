...
namespace Opm
{
    ...
    public:
        ...
        bool iterateWellEqWithControl(const Simulator& modelSimulator,
                                      const double dt,
        ...
        bool iterateWellEqWithSwitching(const Simulator& modelSimulator,
                                        const double dt,
                                        const Well::InjectionControls& inj_controls,
                                        const Well::ProductionControls& prod_controls,
                                        WellState& well_state,
        ...
        /* returns BHP */
        double computeWellRatesAndBhpWithThpAlqProd(const Simulator& model_simulator,
        ...
        void updateIPRImplicit(const Simulator& modelSimulator, WellState& well_state, DeferredLogger& deferred_logger) override;
    ...
    protected:
        ...
        void computePropertiesForWellConnectionPressures(const Simulator& modelSimulator,
                                                         const WellState& well_state,
                                                         WellConnectionProps& props) const;

        void computeWellConnectionDensitesPressures(const Simulator& modelSimulator,
                                                    const WellState& well_state,
                                                    const WellConnectionProps& props,
                                                    DeferredLogger& deferred_logger);

        void computeWellConnectionPressures(const Simulator& modelSimulator,
                                            const WellState& well_state,
                                            DeferredLogger& deferred_logger);
        ...
        void computeWellRatesWithBhpIterations(const Simulator& modelSimulator,
                                              const double& bhp,
                                              std::vector<double>& well_flux,
                                              DeferredLogger& deferred_logger) const override;
        ...
        bool computeWellPotentialsImplicit(const Simulator& model_simulator,
                                           std::vector<double>& well_potentials,
                                           DeferredLogger& deferred_logger) const;               
        ...
        void updateWaterMobilityWithPolymer(const Simulator& model_simulator,
                                            const int perf,
        ...
        void assembleWellEqWithoutIterationImpl(const Simulator& modelSimulator,
                                                const double dt,
                                                const Well::InjectionControls& inj_controls,
                                                const Well::ProductionControls& prod_controls,
                                                WellState& well_state,
        ...
        void calculateSinglePerf(const Simulator& modelSimulator,
                                 const int perf,
                                 WellState& well_state,
        ...
        void checkOperabilityUnderTHPLimit(const Simulator& model_simulator,
                                           const WellState& well_state,
                                           DeferredLogger& deferred_logger) override;

        // updating the inflow based on the current reservoir condition
        void updateIPR(const Simulator& model_simulator,
                       DeferredLogger& deferred_logger) const override;

        // for a well, when all drawdown are in the wrong direction, then this well will not
        // be able to produce/inject .
        bool allDrawDownWrongDirection(const Simulator& model_simulator) const;

        // whether the well can produce / inject based on the current well state (bhp)
        bool canProduceInjectWithCurrentBhp(const Simulator& model_simulator,
                                            const WellState& well_state,
                                            DeferredLogger& deferred_logger);
        ...
        bool openCrossFlowAvoidSingularity(const Simulator& model_simulator) const;
        ...
        void handleInjectivityRate(const Simulator& modelSimulator,
                                   const int perf,
                                   std::vector<EvalWell>& cq_s) const;

        // handle the extra equations for polymer injectivity study
        void handleInjectivityEquations(const Simulator& modelSimulator,
                                        const WellState& well_state,
                                        const int perf,
        ...
        std::optional<double> computeBhpAtThpLimitInj(const Simulator& model_simulator,
                                                      const SummaryState& summary_state,
                                                      DeferredLogger& deferred_logger) const;
