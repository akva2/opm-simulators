...
;
            if (FluidSystem::enableVaporizedWater() && FluidSystem::enableDissolvedGasInWater()) {
                volumeRatio += volumeRatioVapDisWat(rvw, rsw, pressure,
            ...
            if (this->isProducer()) {
                if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                    computeGasOilPerfRateCrossflow(cq_s, perf_rates,
                                                   rs, rv, pressure, rvw, deferred_logger);
                }
    ...
    void
    StandardWell<TypeTag>::
    computeGasOilPerfRate(std::vector<Value>& cq_s,
                          PerforationRates& perf_rates,
                          const Value& rv,
                          const Value& rs,
    ...
    void
    StandardWell<TypeTag>::
    computeGasOilPerfRateCrossflow(const std::vector<Value>& cq_s,
                                   PerforationRates& perf_rates,
                                   const Value& rv,
                                   const Value& rs,
                                   const Value& pressure,
                                   ...
                                   DeferredLogger& deferred_logger) const
    {
        ...
        if (d <= 0.0) {
            deferred_logger.debug(dValueError(d, this->name(),
                                              "computeGasOilPerfRateCrossflow",
                                              rs, rv, pressure));
        } else {
    ...
    void
    StandardWell<TypeTag>::
    computeGasWaterPerfRate(std::vector<Value>& cq_s,
                            PerforationRates& perf_rates,
                            const Value& rvw,
    ...
    Value
    StandardWell<TypeTag>::
    volumeRatioVapDisWat(const Value& rvw,
                         const Value& rsw,
                         const Value& pressure,
                         const std::vector<Value>& cmix_s,
