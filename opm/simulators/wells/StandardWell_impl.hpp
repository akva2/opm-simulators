                    ...
                    DeferredLogger& deferred_logger) const
    {
        ...
        if (drawdown > 0)  {
            ...
            if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                ...
                WellPerforations<FluidSystem,Indices,Scalar,Value>(*this).
                    gasWaterRate(cq_s, perf_rates, rvw, rsw);
            }
        } else {
            // Do nothing if crossflow is not allowed
...
;
            if (FluidSystem::enableVaporizedWater() && FluidSystem::enableDissolvedGasInWater()) {
                volumeRatio += WellPerforations<FluidSystem,Indices,Scalar,Value>(*this).
                                   volumeRatioDisOilVapWat(rvw, rsw, pressure,
            ...
            if (this->isProducer()) {
                if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                    WellPerforations<FluidSystem,Indices,Scalar,Value>(*this).
                        gasOilRateCrossflow(cq_s, perf_rates, rs, rv,
                                            pressure, rvw, deferred_logger);
                }
                if (FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
                    //no oil
                    WellPerforations<FluidSystem,Indices,Scalar,Value>(*this).
                        gasWaterRateCrossflow(cq_s, perf_rates, rvw, rsw);
                }
            }
        }
    }
    ...
    getMobility(const Simulator& ebosSimulator,
                ...
                DeferredLogger& deferred_logger) const
    {
        std::function<Value(const Eval&)> obtain;
        if constexpr (std::is_same_v<Value, Scalar>) {
            obtain = [](const Eval& value)
                     {
                         return getValue(value);
                     };
        } else {
            obtain = [this](const Eval& value)
                     {
