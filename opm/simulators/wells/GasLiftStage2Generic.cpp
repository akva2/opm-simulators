...
GasLiftStage2Generic::GasLiftStage2Generic(DeferredLogger &deferred_logger,
    ...
    , comm_(comm)
    , glo_(schedule.glo(reportStepIdx))
    , debug_{false}
{
}
...
bool
GasLiftStage2Generic::
checkRateAlreadyLimited_(GasLiftWellState& state,
                         bool increase)
{
...
GasLiftSingleWellGeneric::GradInfo
GasLiftStage2Generic::
...
GasLiftSingleWellGeneric::GradInfo
GasLiftStage2Generic::
deleteGrad_(const std::string& name,
            bool increase)
{
...
std::tuple<double, double, double>
GasLiftStage2Generic::
getCurrentWellRates_(const std::string& well_name,
                     const std::string& group_name)
{
    ...
    if (this->stage1_wells_.count(well_name) == 1) {
        GasLiftSingleWellGeneric &gs_well = *(this->stage1_wells_.at(well_name).get());
    ...
    if (success) {
        ...
        if (this->debug_) {
            ...
            this->displayDebugMessage_(msg, group_name);
        }
                             ...
                             std::vector<GasLiftSingleWellGeneric*>& wells)
{
    ...
    for (const std::string& well_name : group.wells()) {
        if (this->stage1_wells_.count(well_name) == 1) {
            GasLiftSingleWellGeneric *well_ptr = this->stage1_wells_.at(well_name).get();
...
mpiSyncGlobalGradVector_(std::vector<GradPair>& grads_global) const
{
    ...
    for (const auto& itr : grads_global) {
...
void
GasLiftStage2Generic::
saveDecGrad_(const std::string& name,
             GradInfo& grad)
{
...
void
GasLiftStage2Generic::
saveIncGrad_(const std::string& name,
             GradInfo& grad)
{
...
std::optional<GasLiftSingleWellGeneric::GradInfo>
GasLiftStage2Generic::
updateGrad_(const std::string& name,
            GradInfo& grad,
            bool increase)
{
    GradMap& map = increase ? this->inc_grads_ : this->dec_grads_;
...
void
GasLiftStage2Generic::OptimizeState::
calculateEcoGradients(std::vector<GasLiftSingleWellGeneric*>& wells,
                      std::vector<GradPair>& inc_grads,
                      std::vector<GradPair>& dec_grads)
{
...
bool
GasLiftStage2Generic::OptimizeState::
checkAtLeastTwoWells(std::vector<GasLiftSingleWellGeneric*>& wells)
{
    int numberOfwells = 0;
    for (auto well : wells){
...
std::pair<std::optional<GasLiftStage2Generic::GradPairItr>,
          std::optional<GasLiftStage2Generic::GradPairItr>>
GasLiftStage2Generic::OptimizeState::
...
void
GasLiftStage2Generic::
redistributeALQ_(std::vector<GasLiftSingleWellGeneric*>& wells,
                 const Group& group,
                 std::vector<GradPair>& inc_grads,
                 std::vector<GradPair>& dec_grads)
{
...
void
GasLiftStage2Generic::SurplusState::
...
bool
GasLiftStage2Generic::SurplusState::
checkEcoGradient(const std::string& well_name,
                 double eco_grad)
{
...
updateRates(const std::string& well_name)
{
    ...
    if (this->parent.well_state_map_.count(well_name) > 0) {
        ...
        GasLiftSingleWellGeneric &gs_well = *(this->parent.stage1_wells_.at(well_name).get());
        const WellInterfaceGeneric &well = gs_well.getStdWell();
