...
WellConstraints::RatioLimitCheckReport
WellConstraints::checkRatioEconLimits(const WellEconProductionLimits& econ_production_limits,
                                      const SingleWellState& ws,
                                      const ParallelWellInfo& parallel_well_info,
                                      DeferredLogger& deferred_logger) const
{
    ...
    if (econ_production_limits.onMaxWaterCut()) {
        this->checkMaxWaterCutLimit(econ_production_limits, ws, parallel_well_info, report);
    }

    if (econ_production_limits.onMaxGasOilRatio()) {
        this->checkMaxGORLimit(econ_production_limits, ws, parallel_well_info, report);
    }

    if (econ_production_limits.onMaxWaterGasRatio()) {
        this->checkMaxWGRLimit(econ_production_limits, ws, parallel_well_info, report);
    }
                            ...
                            DeferredLogger& deferred_logger) const
{
    ...
    RatioLimitCheckReport ratio_report =
        this->checkRatioEconLimits(econ_production_limits, ws,
                                   parallel_well_info,
                                   deferred_logger);
...
void WellConstraints::updateWellTestStatePhysical(const double simulation_time,
                                                  const bool write_message_to_opmlog,
                                                  WellTestState& well_test_state,
                                                  DeferredLogger& deferred_logger) const
{
