...
namespace Opm {
    ...
    BlackoilWellModel(Simulator& ebosSimulator, const PhaseUsage& phase_usage)
        : BlackoilWellModelGeneric<Scalar>(ebosSimulator.vanguard().schedule(),
    ...
    {
        this->terminal_output_ = ((ebosSimulator.gridView().comm().rank() == 0) &&
    ...
    beginTimeStep()
    {
        ...
        OPM_END_PARALLEL_TRY_CATCH_LOG(local_deferredLogger, "beginTimeStep() failed: ",
                                       this->terminal_output_, ebosSimulator_.vanguard().grid().comm());
        ...
        WellGroupHelpers::updateGuideRates(fieldGroup, this->schedule(), summaryState, this->phase_usage_, reportStepIdx, simulationTime,
        ...
        if (this->schedule_[reportStepIdx].has_gpmaint()) {
            ...
            WellGroupHelpers::updateGpMaintTargetForGroups(fieldGroup,
                                                           this->schedule_, regionalAveragePressureCalculator_, reportStepIdx, dt, this->wellState(), this->groupState());
        }
        try {
                                            ...
                                            DeferredLogger& deferred_logger)
    {
        for (const std::string& well_name : this->getWellsForTesting(timeStepIdx, simulationTime)) {
            ...
            WellGroupHelpers::accumulateGroupEfficiencyFactor(this->schedule().getGroup(wellEcl.groupName(), timeStepIdx),
                                                              this->schedule(), timeStepIdx, well_efficiency_factor);
            ...
            try {
                well->wellTesting(ebosSimulator_, simulationTime, this->wellState(), this->groupState(), this->wellTestState(), deferred_logger);
            } catch (const std::exception& e) {
        ...
        if (nw > 0) {
            ...
            for (int w = 0; w < nw; ++w) {
                ...
                if (this->wellTestState().well_is_closed(well_name)) {
                    ...
                    const bool closed_this_step = (this->wellTestState().lastTestTime(well_name) == ebosSimulator_.time());
    ...
    maybeDoGasLiftOptimize(DeferredLogger& deferred_logger)
    {
        ...
        if (do_glift_optimization) {
            ...
            this->gasLiftOptimizationStage2(
                deferred_logger, prod_wells, glift_wells, group_info, state_map,
                ebosSimulator_.episodeIndex());
            if (this->glift_debug) this->gliftDebugShowALQ(deferred_logger);
