...
namespace Opm {
    ...
    BlackoilWellModel(Simulator& modelSimulator, const PhaseUsage& phase_usage)
        : BlackoilWellModelGeneric(modelSimulator.vanguard().schedule(),
                                   modelSimulator.vanguard().summaryState(),
                                   modelSimulator.vanguard().eclState(),
                                   phase_usage,
                                   modelSimulator.gridView().comm())
        , modelSimulator_(modelSimulator)
    {
        terminal_output_ = ((modelSimulator.gridView().comm().rank() == 0) &&
                           EWOMS_GET_PARAM(TypeTag, bool, EnableTerminalOutput));

        local_num_cells_ = modelSimulator_.gridView().size(0);

        // Number of cells the global grid view
        global_num_cells_ = modelSimulator_.vanguard().globalNumCells();

        {
            auto& parallel_wells = modelSimulator.vanguard().parallelWells();
    ...
    template<typename TypeTag>
    BlackoilWellModel<TypeTag>::
    BlackoilWellModel(Simulator& modelSimulator) :
        BlackoilWellModel(modelSimulator, phaseUsageFromDeck(modelSimulator.vanguard().eclState()))
    {}
    ...
    init()
    {
        ...
        gravity_ = modelSimulator_.problem().gravity()[2];

        initial_step_ = true;

        // add the eWoms auxiliary module for the wells to the list
        modelSimulator_.model().addAuxiliaryModule(this);
    ...
    beginReportStep(const int timeStepIdx)
    {
        ...
        const auto& comm = this->modelSimulator_.vanguard().grid().comm();
        ...
        OPM_BEGIN_PARALLEL_TRY_CATCH()
        {
            ...
            this->rateConverter_->template defineState<ElementContext>(this->modelSimulator_);
    ...
    beginTimeStep()
    {
        ...
        if (this->wellStructureChangedDynamically_) {
            ...
            const auto reportStepIdx =
                this->modelSimulator_.episodeIndex();
        ...
        const int reportStepIdx = modelSimulator_.episodeIndex();
        updateAndCommunicateGroupData(reportStepIdx,
                                      modelSimulator_.model().newtonMethod().numIterations());

        this->wellState().updateWellsDefaultALQ(this->wells_ecl_, this->summaryState());
        this->wellState().gliftTimeStepInit();

        const double simulationTime = modelSimulator_.time();
        OPM_BEGIN_PARALLEL_TRY_CATCH();
        {
            // test wells
            ...
            const Grid& grid = modelSimulator_.vanguard().grid();
        ...
        OPM_END_PARALLEL_TRY_CATCH_LOG(local_deferredLogger, "beginTimeStep() failed: ",
                                        terminal_output_, modelSimulator_.vanguard().grid().comm());
        ...
        const auto& summaryState = modelSimulator_.vanguard().summaryState();
        if (alternative_well_rate_init_) {
            ...
            for (auto& well : well_container_) {
                ...
                if (well->isProducer() && !zero_target) {
                    well->updateWellStateRates(modelSimulator_, this->wellState(), local_deferredLogger);
                }
            }
        }
        ...
        try {
            updateWellPotentials(reportStepIdx,
                                 /*onlyAfterEvent*/true,
                                 modelSimulator_.vanguard().summaryConfig(),
        ...
        //update guide rates
        const auto& comm = modelSimulator_.vanguard().grid().comm();
        ...
        if (schedule_[reportStepIdx].has_gpmaint()) {
            for (auto& calculator : regionalAveragePressureCalculator_) {
                calculator.second->template defineState<ElementContext>(modelSimulator_);
            }
            const double dt = modelSimulator_.timeStepSize();
        ...
        try {
            ...
            for (auto& well : well_container_) {
                ...
                if (event || dyn_status_change) {
                    try {
                        well->updateWellStateWithTarget(modelSimulator_, this->groupState(), this->wellState(), local_deferredLogger);
                        well->calculateExplicitQuantities(modelSimulator_, this->wellState(), local_deferredLogger);
                        well->solveWellEquation(modelSimulator_, this->wellState(), this->groupState(), local_deferredLogger);
                                            ...
                                            DeferredLogger& deferred_logger)
    {
        for (const std::string& well_name : this->getWellsForTesting(timeStepIdx, simulationTime)) {
            ...
            if (well->isProducer()) {
                well->updateWellStateRates(modelSimulator_, this->wellState(), deferred_logger);
            }
            ...
            try {
                well->wellTesting(modelSimulator_, simulationTime, this->wellState(), this->groupState(), wellTestState(), deferred_logger);
            } catch (const std::exception& e) {
    ...
    timeStepSucceeded(const double simulationTime, const double dt)
    {
        ...
        for (const auto& well : well_container_) {
            well->updateConnectionTransmissibilityFactor(modelSimulator_, this->wellState().well(well->indexOfWell()));
            well->updateConnectionDFactor(modelSimulator_, this->wellState().well(well->indexOfWell()));
        }
        ...
        rateConverter_->template defineState<ElementContext>(modelSimulator_);
        ...
        checkGEconLimits(
            fieldGroup, simulationTime, modelSimulator_.episodeIndex(), local_deferredLogger);
        checkGconsaleLimits(fieldGroup, this->wellState(),
                            modelSimulator_.episodeIndex(), local_deferredLogger);
    ...
    initializeWellState(const int timeStepIdx)
    {
        ...
        ElementContext elemCtx(modelSimulator_);

        const auto& gridView = modelSimulator_.vanguard().gridView();
        ...
        OPM_END_PARALLEL_TRY_CATCH("BlackoilWellModel::initializeWellState() failed: ", modelSimulator_.vanguard().grid().comm());
        ...
        if (nw > 0) {
            ...
            for (int w = 0; w < nw; ++w) {
                ...
                if (this->wellTestState().well_is_closed(well_name)) {
                    ...
                    const bool closed_this_step = (wellTestState().lastTestTime(well_name) == modelSimulator_.time());
    ...
    doPreStepNetworkRebalance(DeferredLogger& deferred_logger) {
        const double dt = this->modelSimulator_.timeStepSize();
        ...
        do {
            ...
            for (auto& well : this->well_container_) {
                const auto& summary_state = this->modelSimulator_.vanguard().summaryState();
             ...
             const double dt)
    {
        ...
        {
            const int episodeIdx = modelSimulator_.episodeIndex();
    ...
    maybeDoGasLiftOptimize(DeferredLogger& deferred_logger)
    {
        ...
        const double simulation_time = modelSimulator_.time();
        const double min_wait = modelSimulator_.vanguard().schedule().glo(modelSimulator_.episodeIndex()).min_wait();
        ...
        if (do_glift_optimization) {
            ...
            GasLiftGroupInfo group_info {
                ecl_well_map,
                modelSimulator_.vanguard().schedule(),
                modelSimulator_.vanguard().summaryState(),
                modelSimulator_.episodeIndex(),
                modelSimulator_.model().newtonMethod().numIterations(),
                phase_usage_,
                deferred_logger,
                this->wellState(),
                this->groupState(),
                modelSimulator_.vanguard().grid().comm(),
                this->glift_debug
            };
    ...
    {
        auto comm = modelSimulator_.vanguard().grid().comm();
    ...
    {
        const auto& summary_state = modelSimulator_.vanguard().summaryState();
        std::unique_ptr<GasLiftSingleWell> glift
            = std::make_unique<GasLiftSingleWell>(
                *well, modelSimulator_, summary_state,
    ...
    {
        for (auto& well : well_container_) {
            well->assembleWellEq(modelSimulator_, dt, this->wellState(), this->groupState(), deferred_logger);
        }
    }
    ...
    {
        for (auto& well : well_container_) {
            well->prepareWellBeforeAssembling(modelSimulator_, dt, this->wellState(), this->groupState(), deferred_logger);
        }
    }
    ...
    assembleWellEqWithoutIteration(const double dt, DeferredLogger& deferred_logger)
    {
        ...
        for (auto& well: well_container_) {
            well->assembleWellEqWithoutIteration(modelSimulator_, dt, this->wellState(), this->groupState(),
                                                 deferred_logger);
        }
                            ...
                            std::vector<typename SparseMatrixAdapter::MatrixBlock*>& diagMatAddress) const
    {
        ...
        for (const auto& well : well_container_) {
            ...
            for (unsigned perfIdx = 0; perfIdx < rates.size(); ++perfIdx) {
                ...
                modelSimulator_.model().linearizer().setResAndJacobi(res, bMat, rate);
    ...
    {
        const auto& summary_state = modelSimulator_.vanguard().summaryState();
        const int iterationIdx = modelSimulator_.model().newtonMethod().numIterations();
    ...
    calculateExplicitQuantities(DeferredLogger& deferred_logger) const
    {
        ...
        for (auto& well : well_container_) {
            well->calculateExplicitQuantities(modelSimulator_, this->wellState(), deferred_logger);
        }
    }
        ...
        bool changed_well_to_group = false;
        {
            ...
            OPM_BEGIN_PARALLEL_TRY_CATCH()
                for (const auto& well : well_container_) {
                    ...
                    const bool changed_well = well->updateWellControl(modelSimulator_, mode, this->wellState(), this->groupState(), deferred_logger);
                    if (changed_well) {
    ...
    updateWellControlsDomain(DeferredLogger& deferred_logger, const Domain& domain)
    {
        ...
        for (const auto& well : well_container_) {
            if (well_domain_.at(well->name()) == domain.index) {
                const auto mode = WellInterface<TypeTag>::IndividualOrGroup::Individual;
                well->updateWellControl(modelSimulator_, mode, this->wellState(), this->groupState(), deferred_logger);
            }
        }
    }
                         ...
                         DeferredLogger& deferred_logger)
    {
        ...
        for (const auto& well : well_container_) {
            well->updateWellStateWithTarget(modelSimulator_, this->groupState(), this->wellState(), deferred_logger);
        }
    ...
    updateWellTestState(const double& simulationTime, WellTestState& wellTestState) const
    {
        ...
        for (const auto& well : well_container_) {
            ...
            well->checkWellOperability(modelSimulator_, this->wellState(), local_deferredLogger);
                                                  ...
                                                  DeferredLogger& deferred_logger)
    {
        ...
        try {
            well->computeWellPotentials(modelSimulator_, well_state_copy, potentials, deferred_logger);
        }
    ...
    calculateProductivityIndexValues(const WellInterface<TypeTag>* wellPtr,
                                     DeferredLogger& deferred_logger)
    {
        wellPtr->updateProductivityIndex(this->modelSimulator_,
    ...
    prepareTimeStep(DeferredLogger& deferred_logger)
    {
        ...
        const auto episodeIdx = modelSimulator_.episodeIndex();
        ...
        for (const auto& well : well_container_) {
            ...
            if (param_.solve_welleq_initially_ && well->isOperableAndSolvable()) {
                try {
                    well->solveWellEquation(modelSimulator_, this->wellState(), this->groupState(), deferred_logger);
                } catch (const std::exception& e) {
    ...
    updateAverageFormationFactor()
    {
        ...
        const auto& grid = modelSimulator_.vanguard().grid();
    ...
    {
        const auto& grid = modelSimulator_.vanguard().grid();
        const auto& eclProblem = modelSimulator_.problem();
    ...
    {
        return std::max(this->modelSimulator_.episodeIndex(), 0);
    }
    ...
    computeWellTemperature()
    {
        ...
        for (auto wellID = 0*nw; wellID < nw; ++wellID) {
            ...
            for (int_type perf = 0, end_perf = well_perf_data_[wellID].size(); perf < end_perf; ++perf) {
                ...
                const auto& intQuants = modelSimulator_.model().intensiveQuantities(cell_idx, /*timeIdx=*/0);
                const auto& fs = intQuants.fluidState();
