...
bool WellBhpThpCalculator::wellHasTHPConstraints(const SummaryState& summaryState,
                                                 const Well& well_ecl)
{
...
double WellBhpThpCalculator::getTHPConstraint(const SummaryState& summaryState,
                                              const Well& well_ecl)
{
    if (well_ecl.isInjector()) {
        const auto& controls = well_ecl.injectionControls(summaryState);
...
double WellBhpThpCalculator::mostStrictBhpFromBhpLimits(const SummaryState& summaryState,
                                                        const Well& well_ecl)
{
    if (well_ecl.isInjector()) {
        const auto& controls = well_ecl.injectionControls(summaryState);
...
double WellBhpThpCalculator::calculateThpFromBhp(const WellState &well_state,
       ...
       DeferredLogger& deferred_logger)
{
    ...
    if (f_low * f_high > 0.0) {
        ...
        if (it < maxit) {
            ...
            deferred_logger.warning("FAILED_ROBUST_BHP_THP_SOLVE_INOPERABLE",
                                    "Bisect did not find the bhp-point where we produce for well " + name);
            return std::nullopt;
        }
    }
    if constexpr (extraBhpAtThpLimitOutput) {
                         ...
                         DeferredLogger& deferred_logger)
{
    ...
    const auto& table = vfpProperties.getProd()->getTable(controls.vfp_table_number);
    const double vfp_ref_depth = table.getDatumDepth();
    const double thp_limit = WellBhpThpCalculator::getTHPConstraint(summary_state, well_ecl);
    const double dp = wellhelpers::computeHydrostaticCorrection(refDepth, vfp_ref_depth,
                                                                rho, gravity);

    auto fbhp = [&controls, &vfpProperties,
                 thp_limit, dp, alq_value,
                 indexOfWell, useVfpExplicit](const std::vector<double>& rates) {
        assert(rates.size() == 3);
        const auto& wfr =  vfpProperties.getExplicitWFR(controls.vfp_table_number, indexOfWell);
        const auto& gfr = vfpProperties.getExplicitGFR(controls.vfp_table_number, indexOfWell);
        return vfpProperties.getProd()
        ->bhp(controls.vfp_table_number, rates[Water], rates[Oil], rates[Gas], thp_limit, alq_value, wfr, gfr, useVfpExplicit) - dp;
    };
    ...
    auto bhp_max = WellBhpThpCalculator::bhpMax(fflo, well_ecl.name(),
                                                controls.bhp_limit, maxPerfPress,
    ...
    if (!bhp_max.has_value()) {
        deferred_logger.warning("FAILED_ROBUST_BHP_THP_SOLVE_INOPERABLE",
                                ...
                                "find bhp-point where production becomes non-zero for well " + well_ecl.name());
        return std::nullopt;
    }
    const std::array<double, 2> range {controls.bhp_limit, *bhp_max};
    return computeBhpAtThpLimit(frates, fbhp, range, well_ecl.name(), deferred_logger);
}
                     ...
                     DeferredLogger& deferred_logger)
{
    ...
    bool finding_bracket = WellBhpThpCalculator::bisectBracket(eq, range, name, low, high,
    ...
    if (!finding_bracket) {
        ...
        finding_bracket = WellBhpThpCalculator::bruteForceBracket(eq, range, low, high, deferred_logger);
    }

    if (!finding_bracket) {
        deferred_logger.warning("FAILED_ROBUST_BHP_THP_SOLVE_INOPERABLE",
                                "Robust bhp(thp) solve failed due to not being able to "
                                "bracket the bhp solution with the brute force search for " + name);
        return std::nullopt;
    }
...
{
    if (throwOnError)
        return computeBhpAtThpLimitInjImpl<ThrowOnError>(frates,
                                                         summary_state,
                                                         vfpProperties,
                                                         well_ecl,
                                                         rho,
        ...
        return computeBhpAtThpLimitInjImpl<WarnAndContinueOnError>(frates,
                                                                   summary_state,
                                                                   vfpProperties,
                                                                   well_ecl,
                                                                   rho,
                            ...
                            DeferredLogger& deferred_logger)
{
    ...
    // Make the fbhp() function.
    const auto& controls = well_ecl.injectionControls(summary_state);
    const auto& table = vfpProperties.getInj()->getTable(controls.vfp_table_number);
    const double vfp_ref_depth = table.getDatumDepth();
    const double thp_limit = WellBhpThpCalculator::getTHPConstraint(summary_state, well_ecl);
    const double dp = wellhelpers::computeHydrostaticCorrection(refDepth, vfp_ref_depth, rho, gravity);
    auto fbhp = [&controls, &vfpProperties, thp_limit, dp](const std::vector<double>& rates)
    {
        assert(rates.size() == 3);
        return vfpProperties.getInj()
    ...
    for (double flo_sample : flo_samples) {
        ...
        try {
            ...
            deferred_logger.warning("FAILED_ROBUST_BHP_THP_SOLVE_EXTRACT_SAMPLES",
                                    "Robust bhp(thp) solve failed extracting bhp values at flo samples for well " + well_ecl.name());
        }
    }
    ...
    if (low == high) {
        ...
        deferred_logger.warning("FAILED_ROBUST_BHP_THP_SOLVE",
                                "Robust bhp(thp) solve failed for well " + well_ecl.name());
        return std::nullopt;
    }
    try {
        const double solved_bhp = RegulaFalsiBisection<ErrorPolicy>::
                solve(eq, low, high, max_iteration, bhp_tolerance, iteration);
        if constexpr (extraBhpAtThpLimitOutput) {
            OpmLog::debug("*****    " + well_ecl.name() + "    solved_bhp = " + std::to_string(solved_bhp)
