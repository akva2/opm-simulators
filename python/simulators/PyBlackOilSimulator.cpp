...
int PyBlackOilSimulator::currentStep()
{
    ...
    // NOTE: this->model_simulator_->episodeIndex() would also return the current
...
int PyBlackOilSimulator::stepInit()
{
    ...
    if (this->flow_main_) {
        ...
        this->model_simulator_ = this->flow_main_->getSimulatorPtr();
        this->material_state_ = std::make_unique<PyMaterialState<TypeTag>>(this->model_simulator_);
        return result;
    }
    else {
        return exit_code;
